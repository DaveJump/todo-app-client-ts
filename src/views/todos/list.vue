...
class TodoList extends Vue {
  ...
  async getTodoList (data: Obj = {}): Promise<any> {
    try {
      let results = await todosAPI.getTodos({
        data,
        params: {
  ...
  batchOperate (type: string, todo?: Todo | number | string) {
    this.$dialog.confirm({
      title: '提示',
      message: `确认${type === 'update' ? '完成' : '删除'}待办?`
    })
      .then(async () => {
        let method: string = type === 'update' ? 'updateTodos' : 'deleteTodos'
        ...
        try {
          ...
          await todosAPI[method]({
            data: Object.assign(
              {
                todos: todo ? [todo] : this.selectedTodos
              },
